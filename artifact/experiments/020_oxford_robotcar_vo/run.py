...
from pose_csv import read_pose_csv
from execution_guard import require_confirmation_release, require_development_ready, workflow
...
def read_numeric_csv(path: Path) -> tuple[list[str], np.ndarray]:
    """Compatibility entry point; RTK zone metadata remains textual."""
...
def admitted_records(manifest: dict, *, include_confirmation: bool = False):
    if include_confirmation:
        require_confirmation_release()
    records = []
    for record in manifest["records"]:
        if record["split"] != split_for(record["traversal"]):
            raise RuntimeError(f"manifest split mismatch: {record['traversal']}")
        if include_confirmation or record["split"] != "confirm":
            records.append(record)
    return records


class OxfordData:
    def __init__(self, cfg: Config, *, include_confirmation: bool = False):
        if include_confirmation:
            require_confirmation_release()
        self.allowed_splits = ("train", "dev", "confirm") if include_confirmation else ("train", "dev")
        ...
        self.trajectories = {}
        exclusions = []
        for record in admitted_records(manifest, include_confirmation=include_confirmation):
            ...
            try:
                ...
                print(f"loaded recorded traversal {name}", flush=True)
    ...
    def batch(self, split: str, length: int, indices):
        if split not in self.allowed_splits:
            raise RuntimeError(f"split was not admitted for loading: {split}")
...
def run(split: str, cfg: Config, device: torch.device) -> dict:
    if split == "confirm":
        ...
        raise ValueError(f"unsupported Oxford execution split: {split}")
    flow = workflow()
    flow.verify_inputs()
    data = OxfordData(cfg, include_confirmation=(split == "confirm"))
    ...
    for arm in ARMS:
        ...
        for seed in SEEDS:
            ...
            meta = {"arm": arm, "seed": seed, "config": asdict(cfg), **flow.fingerprint()}
            record_path = RUNS / f"evaluation_{split}_{arm}_seed{seed}.json"
            if record_path.exists():
                record = flow.read(record_path)
                flow.checked_checkpoint(checkpoint, meta, device)
                if record["checkpoint_sha256"] != flow.sha(checkpoint) or record["fingerprint"] != flow.fingerprint():
                    raise RuntimeError("completed seed record provenance changed")
                output["arms"][arm][str(seed)] = record
                continue
            if split == "dev" and not checkpoint.exists():
                trained = train_one(data, arm, seed, cfg, device)
                model = trained.pop("model")
                flow.save_checkpoint(checkpoint, {
                    "state_dict": model.state_dict(), "arm": arm, "seed": seed,
                    "config": asdict(cfg), "meta": meta, "training": trained,
                })
            else:
                if not checkpoint.exists():
                    raise FileNotFoundError(f"frozen checkpoint missing: {checkpoint}")
                saved = flow.checked_checkpoint(checkpoint, meta, device)
                ...
                trained = saved["training"]
            record = {"training": trained, "lengths": {}, "fingerprint": flow.fingerprint(),
                      "checkpoint_sha256": flow.sha(checkpoint)}
            for length in EVAL_LENGTHS:
                paths = {
            ...
            output["arms"][arm][str(seed)] = record
            flow.finite_tree(record)
            flow.save(record_path, record)
...
def main() -> None:
    ...
    if destination.exists():
        raise RuntimeError(f"refusing to overwrite completed campaign: {destination}")
    torch.set_num_threads(1)
    result = run(split, cfg, torch.device(args.device))
    workflow().save(destination, result)
