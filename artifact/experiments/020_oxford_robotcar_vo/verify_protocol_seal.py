...
def verify() -> dict:
    # The implementation seal is a new link, not a rewrite of either prior seal.
    from execution_guard import workflow
    flow = workflow()
    flow.verify_implementation()
    return {"passes": True, "original_and_schema_history_verified": True,
            "implementation_sha256": flow.sha(flow.SEAL),
            "authority": "local integrity chain, not external preregistration"}


def verify_schema_stage_snapshot() -> dict:
