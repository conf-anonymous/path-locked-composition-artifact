"""Fail closed against the prospectively frozen ETH3D gate."""
...
def audit(results: dict) -> dict:
    raw = cluster_mean(results["baselines"][PRIMARY_LENGTH]["raw_motor"])
    ...
    path_locking = (
        ...
        and learned_gap >= max(0.05, 0.20 * float(np.mean(learned)))
    )
    ...
    counts = results.get("parameter_counts", {})
    parameter_match = (
        "learned" in counts and "mlp" in counts
        and abs(counts["learned"] - counts["mlp"]) / counts["learned"] < 0.05
    )
    complete = "front_end_summary" in results and parameter_match
    for length in (4, 8, 16, 32):
        ...
        for seed in range(5):
            for length in (4, 8, 16, 32):
                ...
                complete &= all(key in record for key in (
                    "paths", "translation_deltas", "left_vs_raw", "moving_paths",
                    "moving_translation_deltas", "moving_left_vs_raw",
                ))
                complete &= all(
                    path_name in record.get(group, {})
...
def main() -> None:
    ...
    if not result["passes"]:
        raise SystemExit("FROZEN ETH3D GATE FAILED")
