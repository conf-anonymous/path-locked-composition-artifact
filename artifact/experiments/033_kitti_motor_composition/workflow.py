...
def fingerprint():
    ...
    for module in list(sys.modules.values()):
        ...
        if name:
            ...
            # The virtual environment is inside the repository. Lazy package
            # imports must not change the research-source inventory at runtime.
            if path.is_file() and path.is_relative_to(a.ROOT/'experiments') and path.parent!=a.HERE and path.suffix=='.py':
                dependencies.add(path)
    sources=set(a.HERE.glob('*.py'))|set(a.HERE.glob('*.cpp'))|dependencies
    sources|={a.HERE/name for name in ('EXPERIMENT_PROTOCOL.md','IMPLEMENTATION_NOTES.md','PRETRAINING_SEAL_AMENDMENT.md','PREPARATION_PROTOCOL.md','FRONTEND_SMOKE_PROTOCOL.md')}
